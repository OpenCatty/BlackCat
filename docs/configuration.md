...
OAuth authentication settings for providers that require it. See [OAuth Setup](./oauth.md) for detailed walkthroughs.
...
> **Warning:** Antigravity uses Google's internal API. You must set `acceptedToS: true` to acknowledge the Terms of Service risk. See [OAuth Setup](./oauth.md#tos-risk-acknowledgment).
...
See [Zen Coding Plan](./zen-plan.md) for setup details and available models.
...
- [LLM Providers](./providers.md) — Provider-specific setup
- [OAuth Setup](./oauth.md) — OAuth authentication details
- [CLI Configure](./configure-cli.md) — Interactive configuration wizard
