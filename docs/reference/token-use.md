---
summary: "How BlackCat builds prompt context and reports token usage + costs"
read_when:
...
BlackCat tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

BlackCat assembles its own system prompt on every run. It includes:
...
For images, BlackCat downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from BlackCat session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `blackcat status --usage` and `blackcat channels list` show
...
`cacheWrite`. If pricing is missing, BlackCat shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. BlackCat can
...
Anthropic's 1M context window is currently beta-gated. BlackCat can inject the
...
BlackCat skips the `context-1m-*` beta header because Anthropic currently
