...
BlackCat integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- BlackCat owns lifecycle when `channels.signal.autoStart=true`.
...
- BlackCat spawns `imsg rpc` as a child process (legacy iMessage integration).
