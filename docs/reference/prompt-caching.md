...
- With Anthropic API-key auth profiles, BlackCat seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, BlackCat injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
BlackCat exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.blackcat/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$BLACKCAT_STATE_DIR/logs/cache-trace.jsonl`
...
- `BLACKCAT_CACHE_TRACE=1` enables cache tracing.
- `BLACKCAT_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `BLACKCAT_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `BLACKCAT_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `BLACKCAT_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
