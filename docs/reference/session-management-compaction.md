...
This document explains how BlackCat manages sessions end-to-end:
...
BlackCat is designed around a single **Gateway process** that owns session state.
...
BlackCat persists sessions in two layers:
...
- Store: `~/.blackcat/agents/<agentId>/sessions/sessions.json`
- Transcripts: `~/.blackcat/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

BlackCat resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, BlackCat reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
blackcat sessions cleanup --dry-run
blackcat sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.blackcat/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
BlackCat intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (BlackCat consumes the events, but Pi decides when to compact).
...
BlackCat also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, BlackCat bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, BlackCat leaves it alone.
...
- `blackcat status` (CLI)
- `blackcat sessions` / `sessions --json`
...
BlackCat supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- BlackCat strips/suppresses this in the delivery layer.

As of `2026.1.10`, BlackCat also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
BlackCat uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but BlackCat’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `blackcat status`.
