...
BlackCat features that can generate provider usage or paid API calls.
...
- `blackcat status --usage` and `blackcat channels list` show provider **usage windows**
...
BlackCat can pick up credentials from:
...
- `blackcat status --usage`
- `blackcat models status --json`
...
`blackcat models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
