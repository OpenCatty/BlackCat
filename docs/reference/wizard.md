...
This is the full reference for the `blackcat onboard` CLI wizard.
...
<Steps>
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.blackcat/credentials/oauth.json`; auth profiles live in `~/.blackcat/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.blackcat/credentials/oauth.json` (or `$BLACKCAT_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.blackcat/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `blackcat pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `blackcat health`.
    - Tip: `blackcat status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
blackcat onboard --non-interactive \
  --mode local \
  --auth-choice apiKey \
...
```bash
export BLACKCAT_GATEWAY_TOKEN="your-token"
blackcat onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env BLACKCAT_GATEWAY_TOKEN
```
...
```bash
blackcat agents add work \
  --workspace ~/.blackcat/workspace-work \
...
- Stores it under `~/.blackcat/tools/signal-cli/<version>/`.
...
Typical fields in `~/.blackcat/blackcat.json`:
...
`blackcat agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.blackcat/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.blackcat/agents/<agentId>/sessions/`.
