---
summary: "CLI reference for `blackcat configure` (interactive configuration prompts)"
read_when:
...
Tip: `blackcat config` without a subcommand opens the same wizard. Use
`blackcat config get|set|unset` for non-interactive edits.
...
```bash
blackcat configure
blackcat configure --section model --section channels
```
