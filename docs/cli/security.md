---
summary: "CLI reference for `blackcat security` (audit and fix common security footguns)"
read_when:
...
```bash
blackcat security audit
blackcat security audit --deep
blackcat security audit --fix
blackcat security audit --json
```
...
It also emits `security.trust_model.multi_user_heuristic` when config suggests likely shared-user ingress (for example open DM/group policy, configured group targets, or wildcard sender rules), and reminds you that BlackCat is a personal-assistant trust model by default.
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `blackcat.browserConfigEpoch`) and recommends `blackcat sandbox recreate --browser --all`.
...
```bash
blackcat security audit --json | jq '.summary'
blackcat security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
blackcat security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
