---
summary: "CLI reference for `blackcat onboard` (interactive onboarding wizard)"
read_when:
...
```bash
blackcat onboard
blackcat onboard --flow quickstart
blackcat onboard --flow manual
blackcat onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`BLACKCAT_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `blackcat dashboard` (Control UI, no channel setup).
