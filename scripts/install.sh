...
# BlackCat Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one BlackCat."
NODE_MIN_MAJOR=22
NODE_MIN_MINOR=12
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 BlackCat Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://blackcat.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.blackcat.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_blackcat_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.blackcat-* "$npm_root"/blackcat 2>/dev/null || true
}

extract_blackcat_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_blackcat_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/blackcat" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/blackcat"|"/usr/local/bin/blackcat")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/blackcat/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale blackcat symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing blackcat binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing BlackCat package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_blackcat_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "BlackCat npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*blackcat" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_blackcat_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_blackcat_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "BlackCat npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an blackcat binary already exists"
            if [[ -n "$conflict" ]]; then
...
print_usage() {
    cat <<EOF
BlackCat installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/blackcat)
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected BlackCat checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a BlackCat source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_blackcat_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"blackcat"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://blackcat.ai/install.sh | bash"
}
...
ensure_blackcat_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/blackcat" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/blackcat" ]]; then
        ln -sf "$npm_root/blackcat/dist/entry.js" "${npm_bin}/blackcat"
        ui_info "Created blackcat bin link at ${npm_bin}/blackcat"
    fi
    return 0
}

# Check for existing BlackCat installation
check_existing_blackcat() {
    if [[ -n "$(type -P blackcat 2>/dev/null || true)" ]]; then
        ui_info "Existing BlackCat installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make blackcat show as \"command not found\" in new terminals."
...
warn_blackcat_not_found() {
    ui_warn "Installed, but blackcat is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t blackcat 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named blackcat; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_blackcat_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P blackcat 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/blackcat" ]]; then
        echo "${npm_bin}/blackcat"
        return 0
    fi
...
install_blackcat_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/OpenCatty/BlackCat.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing BlackCat from git checkout: ${repo_dir}"
    else
        ui_info "Installing BlackCat from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning BlackCat" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building BlackCat" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/blackcat" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/blackcat"
    ui_success "BlackCat wrapper installed to \$HOME/.local/bin/blackcat"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view blackcat dist-tags.beta 2>/dev/null || true)"
...
install_blackcat() {
    ...
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing BlackCat v${resolved_version}"
    else
        ui_info "Installing BlackCat (${OPENCLAW_VERSION})"
    fi
    local install_spec=""
    ...
    if ! install_blackcat_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_blackcat_paths
        install_blackcat_npm "${install_spec}"
    fi

    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "blackcat" ]]; then
        if ! resolve_blackcat_bin &> /dev/null; then
            ui_warn "npm install blackcat@latest failed; retrying blackcat@next"
            cleanup_npm_blackcat_paths
            install_blackcat_npm "blackcat@next"
        fi
    fi

    ensure_blackcat_bin_link || true

    ui_success "BlackCat installed"
}
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        claw="$(resolve_blackcat_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (blackcat not on PATH yet)"
        warn_blackcat_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.blackcat/workspace-${profile}"
    else
        echo "${HOME}/.blackcat/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.OpenCatty/BlackCat.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run blackcat onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but blackcat not on PATH; skipping onboarding"
        warn_blackcat_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run blackcat onboard to retry"
        return
    }
}

resolve_blackcat_version() {
    local version=""
    local claw="${OPENCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v blackcat &> /dev/null; then
        claw="$(command -v blackcat)"
    fi
    if [[ -n "$claw" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/blackcat/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/blackcat/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
main() {
    ...
    detected_checkout="$(detect_blackcat_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found BlackCat checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    if check_existing_blackcat; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing BlackCat"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g blackcat &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g blackcat 2>/dev/null || true
        ...
        install_blackcat_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/blackcat" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/blackcat"
    ...
    OPENCLAW_BIN="$(resolve_blackcat_bin || true)"
    ...
    installed_version=$(resolve_blackcat_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🐱 BlackCat installed successfully (${installed_version})!"
    else
        ui_celebrate "🐱 BlackCat installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/blackcat"
        ui_kv "Update command" "blackcat update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://blackcat.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running blackcat doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
            ...
            ui_info "No TTY; run blackcat doctor and blackcat plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run blackcat onboard later"
        else
            local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.OpenCatty/BlackCat.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (blackcat not on PATH yet)"
                    warn_blackcat_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run blackcat onboard to finish setup"
            return 0
        fi
    fi

    if command -v blackcat &> /dev/null; then
        local claw="${OPENCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_blackcat_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (blackcat daemon restart)"
            else
                ...
                if OPENCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ...
                    ui_warn "Gateway restart failed; try: blackcat daemon restart"
                fi
            fi
        fi
    fi
