...
import { classifyMessage } from "../../blackcat/router.js";
...
export async function getReplyFromConfig(
  ...
  // BlackCat role routing: classify message and override agentId
  try {
    const messageText = ctx.Body ?? ctx.BodyForAgent ?? "";
    if (messageText) {
      const role = classifyMessage(messageText);
      agentId = role.agentId;
    }
  } catch {
    // Classification failed — fall through to original agentId
  }
